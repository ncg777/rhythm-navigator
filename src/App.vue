<template>
  <div class="min-h-screen flex flex-col">
    <header class="sticky top-0 z-30 border-b border-cyan-300/20 bg-slate-950/85 backdrop-blur-xl">
      <div class="mx-auto max-w-7xl px-2 sm:px-4 lg:px-8 py-2 space-y-2">
        <div class="flex items-center gap-2 min-w-0">
          <h1 class="app-title flex-1 min-w-0" :title="`Rhythm Navigator v${appVersion}`">
            Rhythm Navigator <span class="app-version">v{{ appVersion }}</span>
          </h1>

          <div class="header-actions">
            <span class="preset-state-pill" :class="presetStateClass">{{ dirtyStateLabel }}</span>
              ...
              <template #default="{ close }">
                <div class="menu-stack">
                  <div class="menu-status">
                    <div class="menu-status-title">{{ activePresetName }}</div>
                  ...
                  <button class="menu-btn" @click="onSaveAsCurrent(close)">📌 Save current as preset</button>
                  ...
                  <div v-else class="menu-scroll">
                    <div v-for="preset in presets" :key="preset.id" class="menu-item-row">
                      <button class="menu-link" @click="onLoadPreset(preset.id, close)">
                        {{ preset.id === activePresetId ? '✅ ' : '' }}{{ preset.name }}
                      </button>
                      <div class="flex items-center gap-1">
                        <button class="menu-icon-btn" title="Overwrite this preset" @click="presetStore.overwritePreset(preset.id)">♻️</button>
        ...
        <div class="glass rounded px-2 sm:px-3 py-2 w-full">
          <TransportBar />
        </div>
      </div>
    </header>
    ...
    <Modal :open="presetBrowserOpen" @close="presetBrowserOpen = false">
      <template #title><h3 class="text-lg font-semibold">Preset Browser</h3></template>
      <PresetManagerPanel />
    </Modal>
...
const presetStateClass = computed(() => {
  if (dirtyState.value === 'modified') return 'preset-state-pill--modified'
  if (dirtyState.value === 'clean') return 'preset-state-pill--clean'
  return 'preset-state-pill--unsaved'
})
...
function onLoadPreset(id: string, close: () => void) {
...
.preset-state-pill--modified {
  border-color: rgba(251, 191, 36, 0.6);
  color: #fcd34d;
}

.preset-state-pill--clean {
  border-color: rgba(74, 222, 128, 0.6);
  color: #86efac;
}

.preset-state-pill--unsaved {
